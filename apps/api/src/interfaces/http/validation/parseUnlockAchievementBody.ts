...
import type { UnlockAchievementInput } from "../../../application/usecase/achievement/UnlockAchievement.js";
...
export function parseAchievementSlugParam(value: string): string {
  const trimmed = value.trim();

  if (!SLUG_PATTERN.test(trimmed)) {
    throw new ValidationError("Achievement slug must be a valid slug");
  }

  return trimmed;
}

export function parseUnlockAchievementBody(body: unknown): UnlockAchievementInput {
